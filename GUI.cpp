...
Button::Button(int left, int top, int right, int bottom, void (*function)(void*), const char* text, int text_size)
{
...
void WidgetGroup::ClickOn(int x, int y, void* arg)
{
	for (int i = 0; i < children.size(); i++)
	{
		...
		if (PointInRect(Rect(child.x,child.y, child.x + child.widget->width, child.y + child.widget->height), x, y))
			child.widget->ClickOn(x - child.x, y - child.y, arg);
		else
			child.widget->ClickOff(x - child.x, y - child.y, arg);
	}
}
void WidgetGroup::ClickOff(int x, int y, void* arg)
{
	for (auto& child : children)
		child.widget->ClickOff(x, y, arg);
}
...
WidgetBitmapButton::WidgetBitmapButton(int width, int height, void(*function)(void*), Texture* tTexture, int nFrame)
{
	this->width = width;
	this->height = height;
	this->function = function;
...
WidgetButton::WidgetButton(int width, int height, void (*function)(void*), const char* text, int text_size)
{
	this->width = width;
	this->height = height;
	this->function = function;
