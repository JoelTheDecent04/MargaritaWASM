...
Enemy::Enemy(float fX, float fY)
	: Entity(Textures::Enemy, fX, fY, Type::Enemy, Category::ENEMY)
{
	nCollisionCategories = Category::ENEMY | /*Category::PROJECTILE |*/ Category::PLAYER | Category::STRUCTURE;
...
Entity::Status Enemy::Update(float deltatime)
{
	...
	else
	{
		...
		if (dist_to_player < 400.0f && bShoot)
			SpaceGame::Instance().Entities.Add(new LaserBeam(fX, fY, 1000.f * (player.fX - fX) / dist_to_player, 1000.f * (player.fY - fY) / dist_to_player, LaserBeam::LaserType::DamagePlayers));
...
void Enemy::OnDestroy()
{
	...
	if (randomf() <= 0.1f) {
		SpaceGame::Instance().Entities.Add(new CoinReward(fX, fY));
