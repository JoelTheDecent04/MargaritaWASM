...
Particle::Particle(float fSize, SDL_Color colour, float fX, float fY, float fSpeedX, float fSpeedY)
	: Entity(0, fX, fY, Type::Particle, Category::DECORATION)
{
	this->fSize = fSize;
	this->colour = colour;
...
void Particle::CreateParticles(float fX, float fY, SDL_Color colour)
{
	for (int i = 0; i < 10; i++) {
		...
		SpaceGame::Instance().Entities.Add(new Particle(5.0f + randomf() * 10.0f, colour, fX + 10.0f * fCos, fY + 10.0f * fSin, fCos * 80.0f, fSin * 120.0f));
	}
}
