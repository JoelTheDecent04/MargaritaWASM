...
Player::Player(float fX, float fY)
	: Entity(Textures::Character, fX, fY, Type::Player, Category::PLAYER)
{
	fHealth = 200.0f;
	fMaxHealth = 200.0f;
	fEnergy = 150.0f;
...
//-------------------------MOVEMENT-----------------------------------------
	...
	if (fMoney >= 150.0f && SpaceGame::Instance().GetItem("Bomb") == nullptr)
		SpaceGame::Instance().Items.Add(new BombWeapon(this, 1));
...
List<PlayerUpgrade> Player::GetPossibleUpgrades()
{
	List<PlayerUpgrade> upgrades;

	if (fMovementSpeed < fMaxMovementSpeed) {
		upgrades.Add(new PlayerUpgrade(fMovementSpeed, fMaxMovementSpeed, 10.0f, 50, "Movement Speed"));
	}

	if (fHealthRegeneration < fMaxHealthRegeneration) {
		upgrades.Add(new PlayerUpgrade(fHealthRegeneration, fMaxHealthRegeneration, 10.0f, 150.0f, "Health Regeneration"));
	}

	if (fEnergyRechargeSpeed < fMaxEnergyRechargeSpeed) {
		upgrades.Add(new PlayerUpgrade(fEnergyRechargeSpeed, fMaxEnergyRechargeSpeed, 1.0f, 150, "Energy Recharge"));
	}

	if (fMaxHealth < fMaxHealthUpgrade) {
		upgrades.Add(new PlayerUpgrade(fMaxHealth, fMaxHealthUpgrade, 50.0f, 150, "Max Health"));
	}
